...
public class LoginFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        if(session.getAttribute("userid")!=null){ // 如果当前session中有userid属性，则表示当前用户登录过
            ...
            request.getRequestDispatcher("login.jsp").forward(servletRequest,servletResponse); //服务器跳转，将本次请求通过requestDispather请求转发器(服务器上)，转发给login.jsp，转而由login.jsp来处理本次request请求，此时所有的操作都是在同一个request中完成的，也都是在服务器上完成的，由于只有一个request，所以过滤器也只会作用一次。
